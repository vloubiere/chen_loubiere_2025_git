...
# Output files ----
meta[, mean_contrib_file:= file.path("db/contributions/ATAC", paste0(dataset, "_model_mean_contrib_scores_", tissue, ".rds")), .(dataset, tissue)]
...
meta[, {
  if(overwrite | !file.exists(mean_contrib_file)) {
    ...
    res <- res[, .(score= mean(score)), keyby= .(idx, seqnames, start, base, strand)]
    # Collapse
    res <- res[, .(start= start[1], seq= paste0(base, collapse= ""), score= list(score)), keyby= .(idx, seqnames, strand)]
    res[, end:= start+nchar(seq)-1]
    # Simplify
    res <- res[, .(seqnames, start, end, strand, seq, score)]
    # Collapse
