...
meta[, {
  ...
  # Import observed file ----
  obs <- fread(obs_file)
  
  # Compute TL enhancer labels ----
  obs[, ACC:= ID %in% .c[(ATAC_ov), name]] # Accessible in the tissue
  obs[, TL1:= ACC & ID %in% .c[(K27Ac_ov), name]]
  obs[, TL2:= ACC & ID %in% .c[(K27Ac_ov & K4me1_ov), name]]
  obs[, TL3:= ACC & ID %in% .c[(K27Ac_ov & K4me1_ov & !K4me3_ov), name]]
  obs[, TL4:= ACC & ID %in% .c[(!ext_prom), name]]
  obs[, TL5:= ACC & ID %in% .c[(!ext_prom) & (!CTCF_ov), name]]
  obs[, TL6:= ACC & ID %in% .c[(!ATAC_other_tissue), name]]
  obs[, TL7:= TL3 & TL6] # Combine HTMs and tissue-specific ATAC
  ...
  # Save observed and fasta files with the new labels ----
  obs[, {
